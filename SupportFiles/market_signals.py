...
def build_dashboard_df(stock_df: pd.DataFrame,
    ...
    rows = [
        # ── Header ─────────────────────────────────────────────────────────
        [f"TechnoFunda Market Analysis  [{market}]",  ""],
        ["Generated",           run_time],
        ["Universe",            f"{len(stock_df)} stocks  |  Benchmark: {'NIFTY 50' if is_nse else 'S&P 500'}"],
        ["",                    ""],
        # ── Signal Summary ─────────────────────────────────────────────────
        ["── RS SIGNAL SUMMARY ──",   ""],
        ["🌟 Prime / Triple Confirmed",  sb],
        ["✅ Bullish",            buy],
        ["🔴 Bearish",            sel],
        ["⬜ Neutral",            neu],
        ["",                    ""],
        ["── MULTI-TIMEFRAME SIGNAL SUMMARY ──", ""],
        ["Medium-Term Trend (MST) — Active",    mst_b],
        ["Medium-Term Trend (MST) — Building",  mst_w],
        ["Long-Term Trend (LST) — Active",      lst_b],
        ["Long-Term Trend (LST) — Building",    lst_w],
        ["Weekly Momentum (RS30) — Active",     r30_b],
        ["Weekly Momentum (RS30) — Building",   r30_w],
        ["",                    ""],
        ["── RISK QUALITY (Bullish signals only) ──", ""],
        ["Grade A  — Tight risk (Ideal)",       sl_a],
        ["Grade B  — Good risk control",        sl_b_g],
        ["Grade C  — Acceptable risk",          sl_c],
        ["",                    ""],
        # ── How signals are generated ──────────────────────────────────────
        ["── HOW SIGNALS ARE GENERATED ──", ""],
        ["Relative Strength",   "Stocks are ranked by how well they perform vs their market index and sector peers across multiple timeframes. Sustained outperformance identifies leading stocks."],
        ["Momentum",            "Stocks trending upward across short, medium, and long timeframes score higher. Momentum persists — rising stocks tend to keep rising."],
        ["Trend Confirmation",  "A proprietary trend filter confirms whether a stock is in a sustained uptrend before a signal is generated. This reduces noise from short-term market moves."],
        ["Sector Alignment",    "A stock earns additional weight when its sector is also strong. Top-down analysis — market → sector → stock — improves signal consistency."],
        ["Fundamental Quality", "Revenue growth, profit growth, and return on equity contribute to the final ranking. Strong fundamentals support sustained price momentum."],
        ["Signal Types",        "Three signal types cover different holding periods: Medium-Term Swing, Long-Term Momentum, and Weekly Trend-Following. Each targets a distinct risk-return profile."],
        ["Risk Grade",          "Each signal includes a structure-based risk grade (A to F). Grade A = very tight stop loss. Lower risk + strong signal = higher overall setup quality."],
        ["Composite Ranking",   "Relative strength, trend quality, and fundamental factors are combined into a single proprietary composite score. Higher score = stronger, higher-quality setup."],
        ["",                    ""],
        # ── Sector ranking ─────────────────────────────────────────────────
        ["── SECTOR RANKING (by Relative Strength, best to worst) ──", ""],
    ]
