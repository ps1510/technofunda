"""
TAIWAN MARKET ANALYSIS — HTML-ONLY EDITION  v2.0
market_tw_gsht.py  — NO Google Sheets, generates TW.html only

Universe  : tw_twselist.csv
Benchmark : ^TWII
Timezone  : CST
Output    : TW.html
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "tw_all_stocks_master.csv")
...
TW_INDEX          = "^TWII"
TW_INDEX_FALLBACK = "^TWSE"

TW_INDUSTRY_TO_SECTOR = {
...
TW_BREADTH_INDICES = {
    "TAIEX": {"yahoo": "^TWII", "csv": "tw_twselist.csv"},
}

TW_SNAPSHOT_TICKERS = [
    {"name": "TAIEX",                   "ticker": "^TWII",         "type": "Index"},
    {"name": "CSI 300",                 "ticker": "000300.SS",     "type": "Index"},
    {"name": "Nikkei 225",              "ticker": "^N225",         "type": "Index"},
    {"name": "TWD/USD",                 "ticker": "TWD=X",         "type": "Forex"},
    {"name": "USD/TWD",                 "ticker": "USDTWD=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y TWD Yield",           "ticker": "^TNX",          "type": "Bond"},
...
def load_tw_universe():
    ...
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "tw_twselist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(TW_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# Taiwan sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
TW_SECTORS = {
...
def fetch_tw_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in TW_SECTORS.items():
    ...
    print(f"  ✅ Taiwan Sector prices: {len(result)}/{len(TW_SECTORS)}")
    return result
...
def build_tw_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in TW_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  TAIWAN MARKET — HTML-ONLY  v2.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M CST')}")
    print("="*68+"\n")
    t0 = time.time()

    print(f"Loading Taiwan universe ..."); universe = load_tw_universe()
    if universe.empty: print("Empty universe — aborting."); return

    print(f"Fetching index ({TW_INDEX}) ...")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(TW_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  {TW_INDEX} empty, trying {TW_INDEX_FALLBACK} ...")
        raw = yf.download(TW_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  Cannot fetch Taiwan index"); return
    cl = raw["Close"]
    ...
    sector_prices = fetch_tw_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, TW_SECTORS)
    ...
    breadth_df = build_market_breadth(price_data, index_prices, TW_BREADTH_INDICES, INDEX_DATA_DIR, market="tw")
    ...
    stock_df = build_stock_strength(universe, price_data, index_prices, sector_prices, patterns_by_sym,
        market="tw", fetch_financials=FETCH_FINANCIALS,
        ohlcv_dict=ohlcv_dict if ENABLE_SIGNALS else {}, primary_rs=PRIMARY_RS_PERIOD)
    top_buy_df  = build_top_picks_buy(stock_df, sec_str_df, market="tw", primary_rs=PRIMARY_RS_PERIOD)
    top_sell_df = build_top_picks_sell(stock_df, sec_str_df, market="tw", primary_rs=PRIMARY_RS_PERIOD)
    chart_df    = build_chart_patterns_df(patterns_list, stock_df, market="tw")
    trade_df    = build_trade_setups(stock_df, sec_str_df, market="tw", primary_rs=PRIMARY_RS_PERIOD)
    sleeve_df   = build_rs_sleeve_list(stock_df, universe, INDEX_DATA_DIR, market="tw", run_time=run_time,
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "tw", run_time, primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="TW")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("Building Taiwan HTML report ...")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "TW.html")
        build_html_report(
            market="tw", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="tw",
    ...
    print(f"\n{'='*68}")
    print(f"  COMPLETE!  |  {elapsed:.0f}s  |  TW.html")
    print("="*68)
