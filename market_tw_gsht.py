"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  TAIWAN MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                ║
║  market_tw_gsht.py  — GitHub Actions compatible                  ║
║                                                                            ║
║  Universe  : tw_twselist.csv                                         ║
║  Benchmark : ^TWII                                                ║
║  Timezone  : CST                                                    ║
║  Sheet URL : GOOGLE_SHEET_URL_TW env var                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_TW")
    or "https://docs.google.com/spreadsheets/d/YOUR_TW_SHEET_ID/edit"
)
GSCOPE = [
...
def load_tw_universe():
    ...
    df["Sector"] = df["Sector"].map(TW_INDUSTRY_TO_SECTOR).fillna(df.get("Sector","General"))
...
def fetch_tw_sector_prices():
    """Fetch TAIWAN sector ETF prices (returns empty dict — no local ETFs)."""
    sector_prices = {}
    for sec, cfg in TW_SECTORS.items():
        tkr = cfg.get("yahoo")
        if not tkr:
            continue
        try:
...
def main():
    ...
    print("\n" + "═"*68)
    print(f"  TAIWAN MARKET — GOOGLE SHEETS EDITION  v1.0")
    ...
    print(f"\n📡 Taiwan Sector ETFs …"); sector_prices = fetch_tw_sector_prices()
    ...
    print("\n📸 Market Snapshot …");   snap_df    = build_tw_snapshot()
    ...
    print("📊 Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, TW_BREADTH_INDICES, INDEX_DATA_DIR, market="TW")
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "TW", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)

    # ── BUILD HTML ──────────────────────────────────────────────────────────
    print("\n🌐 Building Taiwan HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "Taiwan_Market_Analysis.html")
        build_html_report(
    ...
    write_dashboard_tab(ss, dashboard_df, "TW");             time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "TW");                   time.sleep(TAB_DELAY)
