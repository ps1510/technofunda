"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  HONG KONG MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_hk_gsht.py  — NO Google Sheets, generates HK.html only       ║
║                                                                              ║
║  Universe  : hk_hsilist.csv                                                           ║
║  Benchmark : ^HSI                                                         ║
║  Timezone  : HKT                                                            ║
║  Output    : HK.html                                                    ║
║                                                                              ║
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "hk_all_stocks_master.csv")
...
# ─────────────────────────────────────────────────────────────────────────────
#  HONG KONG MARKET CONFIG
# ─────────────────────────────────────────────────────────────────────────────

HK_INDEX          = "^HSI"
HK_INDEX_FALLBACK = "^HSCE"

HK_INDUSTRY_TO_SECTOR = {
...
HK_BREADTH_INDICES = {
    "Hang Seng": {"yahoo": "^HSI", "csv": "hk_hsilist.csv"},
}

HK_SNAPSHOT_TICKERS = [
    {"name": "Hang Seng",               "ticker": "^HSI",          "type": "Index"},
    {"name": "HSCEI (H-Share)",         "ticker": "^HSCE",         "type": "Index"},
    {"name": "CSI 300",                 "ticker": "000300.SS",     "type": "Index"},
    {"name": "HKD/USD",                 "ticker": "HKD=X",         "type": "Forex"},
...
def load_hk_universe():
    ...
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "hk_hsilist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(HK_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# Hong Kong sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
HK_SECTORS = {
...
def fetch_hk_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in HK_SECTORS.items():
    ...
    print(f"  ✅ Hong Kong Sector prices: {len(result)}/{len(HK_SECTORS)}")
    return result
...
def build_hk_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in HK_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  HONG KONG MARKET — HTML-ONLY EDITION  v1.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M HKT')}")
    print(f"  Stocks:{MAX_STOCKS}  Patterns:{PATTERN_MAX}  Signals:{SIGNAL_MAX_STOCKS}  RS:{PRIMARY_RS_PERIOD}d")
    print("="*68+"\n")
    t0 = time.time()

    print(f"\U0001F4C2 Loading Hong Kong universe …"); universe = load_hk_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({HK_INDEX}) …")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(HK_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  \u26a0 {HK_INDEX} empty, trying {HK_INDEX_FALLBACK} …")
        raw = yf.download(HK_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, HK_SECTORS)
    ...
    run_time = datetime.now(tz_obj).strftime(f"%d %B %Y %H:%M HKT")

    print("\n\U0001F4F8 Market Snapshot …");   snap_df    = build_hk_snapshot()
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, HK_BREADTH_INDICES, INDEX_DATA_DIR, market="hk")
    ...
    stock_df = build_stock_strength(
        ...
        market="hk", fetch_financials=FETCH_FINANCIALS,
    ...
    print("\U0001F3C6 Top Picks – Buy …");    top_buy_df  = build_top_picks_buy(
        stock_df, sec_str_df, market="hk", primary_rs=PRIMARY_RS_PERIOD)
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "hk", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        html_path = os.path.join(SCRIPT_DIR, "HK.html")
        build_html_report(
            market="hk", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    print(f"\n{'='*68}")
    print(f"  \u2705  COMPLETE!  |  \u23f1 {elapsed:.0f}s  |  \U0001F4C4 HK.html")
