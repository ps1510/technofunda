"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  BRAZIL MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                     ║
║  market_br_gsht.py  — GitHub Actions compatible                           ║
║                                                                            ║
║  Universe  : br_bovespalist.csv  (B3 - Bolsa Brasil, Yahoo suffix .SA)               ║
║  Benchmark : EWZ                              ║
║  Sectors   : Brazil sector ETFs                                       ║
║  Timezone  : BRT                              ║
║  Sheet URL : GOOGLE_SHEET_URL_BR env var                                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_BR")
    or "https://docs.google.com/spreadsheets/d/YOUR_BR_SHEET_ID/edit"
)
GSCOPE = [
...
BR_INDEX          = "EWZ"      # Country benchmark ETF (USD-denominated where applicable)
...
BR_BREADTH_INDICES = {
    "Ibovespa": {"yahoo": "^BVSP", "csv": "br_bovespalist.csv"},
    "IBRX 100": {"yahoo": "^BVSP", "csv": None},
}
...
BR_SNAPSHOT_TICKERS = [
    ...
    {"name": "Brazil 10Y",       "ticker": "^TNX",     "type": "Bond"},
    ...
    {"name": "Soybean",          "ticker": "ZS=F",     "type": "Commodity"},
]
...
def write_sleeve_tab(ss, sleeve_df, market="BR"):
    title = "📋 RS Sleeves"
...
def fetch_br_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in BR_SECTORS.items():
...
def build_br_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in BR_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    print("\n" + "═"*68)
    print("  BRAZIL MARKET — GOOGLE SHEETS EDITION  v1.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M BRT')}")
    ...
    print("📂 Loading Brazil universe …"); universe = load_br_universe()
    if universe.empty: print("❌ Empty universe — aborting."); return

    print(f"\n📡 Fetching index ({BR_INDEX}) …")
    end_dt=datetime.today()+timedelta(days=1); start_dt=end_dt-timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(BR_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  ⚠ {BR_INDEX} empty, trying {BR_INDEX_FALLBACK} …")
        raw = yf.download(BR_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print("  ❌ Cannot fetch Brazil index"); return
    cl = raw["Close"]
    if isinstance(cl, pd.DataFrame): cl=cl.squeeze()
    index_prices = _normalize(cl.dropna())
    print(f"  ✅ Index: {len(index_prices)} days")

    print("\n📡 Brazil Sector ETFs …"); sector_prices = fetch_br_sector_prices()
    ...
    run_time = datetime.now(tz_obj).strftime("%d %B %Y %H:%M BRT")
    ...
    try:
        ...
        html_path = os.path.join(SCRIPT_DIR, "Brazil_Market_Analysis.html")
        build_html_report(
    ...
    write_dashboard_tab(ss, dashboard_df, "BR");                  time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "BR");                        time.sleep(TAB_DELAY)
