"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  GERMANY MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                     ║
║  market_de_gsht.py  — GitHub Actions compatible                           ║
║                                                                            ║
║  Universe  : de_daxlist.csv  (XETRA / Frankfurt, Yahoo suffix .DE)               ║
║  Benchmark : EXS1.DE                              ║
║  Sectors   : Germany sector ETFs                                       ║
║  Timezone  : CET                              ║
║  Sheet URL : GOOGLE_SHEET_URL_DE env var                                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_DE")
    or "https://docs.google.com/spreadsheets/d/YOUR_DE_SHEET_ID/edit"
)
GSCOPE = [
...
DE_BREADTH_INDICES = {
    "DAX 40":   {"yahoo": "^GDAXI",  "csv": "de_daxlist.csv"},
    "MDAX":     {"yahoo": "^MDAXI",  "csv": None},
    "TecDAX":   {"yahoo": "^TECDAX", "csv": None},
}

DE_SNAPSHOT_TICKERS = [
    {"name": "DAX 40",          "ticker": "^GDAXI",    "type": "Index"},
    {"name": "MDAX",            "ticker": "^MDAXI",    "type": "Index"},
    {"name": "TecDAX",          "ticker": "^TECDAX",   "type": "Index"},
...
def write_sleeve_tab(ss, sleeve_df, market="DE"):
    title = "📋 RS Sleeves"
...
def load_de_universe():
    ...
    df["Yahoo"]    = df["Symbol"] + ".DE"     # Yahoo Finance ticker suffix
...
def fetch_de_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in DE_SECTORS.items():
...
def build_de_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in DE_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    print("\n" + "═"*68)
    print("  GERMANY MARKET — GOOGLE SHEETS EDITION  v1.0")
    ...
    print("📂 Loading Germany universe …"); universe = load_de_universe()
    if universe.empty: print("❌ Empty universe — aborting."); return

    print(f"\n📡 Fetching index ({DE_INDEX}) …")
    end_dt=datetime.today()+timedelta(days=1); start_dt=end_dt-timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(DE_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  ⚠ {DE_INDEX} empty, trying {DE_INDEX_FALLBACK} …")
        raw = yf.download(DE_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print("  ❌ Cannot fetch Germany index"); return
    cl = raw["Close"]
    if isinstance(cl, pd.DataFrame): cl=cl.squeeze()
    index_prices = _normalize(cl.dropna())
    print(f"  ✅ Index: {len(index_prices)} days")

    print("\n📡 Germany Sector ETFs …"); sector_prices = fetch_de_sector_prices()
    ...
    print("\n📸 Market Snapshot …");   snap_df    = build_de_snapshot()
    ...
    print("📊 Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, DE_BREADTH_INDICES, INDEX_DATA_DIR, market="DE")
    ...
    stock_df = build_stock_strength(
        ...
        market="DE", fetch_financials=FETCH_FINANCIALS,
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "DE", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)

    TAB_DELAY = 8
    print("\n📊 Writing to Google Sheets …")
    write_dashboard_tab(ss, dashboard_df, "DE");                  time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "DE");                        time.sleep(TAB_DELAY)
    ...
    print("\n🌐 Building Germany HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "Germany_Market_Analysis.html")
        build_html_report(
            market="DE", snapshot_df=snap_df, sector_str_df=sec_str_df,
