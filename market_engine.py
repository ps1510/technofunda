...
def build_market_snapshot(market):
    ...
    try:
        raw=safe_download(syms,days=10,auto_adjust=True,progress=False,threads=True)
...
def build_stock_strength(universe, price_data, index_prices, sector_prices,
    ...
    for i,(_,ur) in enumerate(universe.iterrows()):
        ...
        _fin_est = 0
        for _c,_t,_p in [("SalesYoY",15,2),("PATYoY",15,2),("ROE",15,2),("Margin",10,1)]:
            v = fin.get(_c, np.nan)
            try:
                if not np.isnan(float(v)) and float(v) >= _t: _fin_est += _p
            except: pass
        try:
    ...
    if "Action_Tier" in df.columns:
        df["Action_Tier"] = df.apply(lambda r: compute_action_tier(
            r["Signal"], r["Enhanced"],
            r.get("MST_Signal","Neutral"), r.get("LST_Signal","Neutral"),
            r.get("RS30_Signal","Neutral"), r.get("Supertrend","N/A"),
            int(r["Fin_Score"])
        ), axis=1)
