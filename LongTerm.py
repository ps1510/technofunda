# ═══════════════════════════════════════════════════════════════════════════
#  Chartink Scan → Google Sheets   (v2.0 — GitHub Actions ready)
#
#  HOW TO CREATE A NEW SCAN (e.g. FnO.py):
#    1. Copy this entire file, rename it  →  FnO.py
#    2. Change the 3 CONFIG lines below
#    3. Create Files/Conditions_FnO/ and put your .txt files in it
#    4. Add a new job block in market_analysis.yml  (template is at the bottom)
...
date_string      = datetime.now().strftime("%Y%m%d_%H%M")   # moved up — fixes NameError bug
...
# ── STEP 1: Fetch all condition files from folder ──────────────────────────
print(f"\n{'='*60}")
...
for condition_file in sorted(os.listdir(conditions_dir)):   # sorted = consistent order
...
if dataframes:
    ...
    # Format for Sheets: list of [value] rows
...
if sheet_url:
    try:
        ...
        # Get or create a worksheet tab named after SCAN_NAME
        try:
            ws = sh.worksheet(SCAN_NAME)
        except gspread.exceptions.WorksheetNotFound:
            ws = sh.add_worksheet(title=SCAN_NAME, rows=3000, cols=5)

        # Clear old data, write timestamp header + fresh data
        ws.clear()
        timestamp_header = [[f"{SCAN_NAME}  |  Run: {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC"]]
        ws.update("A1", timestamp_header)
        ...
        raise   # fail the GitHub Actions job so you get an email alert
else:
    # Fallback: save locally (useful for local testing)
...
# ── STEP 4: Log any failed conditions ────────────────────────────────────
