"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  FRANCE MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                     ║
║  market_fr_gsht.py  — GitHub Actions compatible                           ║
║                                                                            ║
║  Universe  : fr_cac40list.csv  (Euronext Paris, Yahoo suffix .PA)               ║
║  Benchmark : CAC.PA                              ║
║  Sectors   : France sector ETFs                                       ║
║  Timezone  : CET                              ║
║  Sheet URL : GOOGLE_SHEET_URL_FR env var                                   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_FR")
    or "https://docs.google.com/spreadsheets/d/YOUR_FR_SHEET_ID/edit"
)
GSCOPE = [
...
FR_INDEX          = "CAC.PA"      # Country benchmark ETF (USD-denominated where applicable)
FR_INDEX_FALLBACK = "^FCHI"     # Local index fallback
...
def write_sleeve_tab(ss, sleeve_df, market="FR"):
    title = "📋 RS Sleeves"
...
def load_fr_universe():
    ...
    df["Yahoo"]    = df["Symbol"] + ".PA"     # Yahoo Finance ticker suffix
    ...
    print(f"  ✅ France Universe: {len(df)} stocks loaded")
...
def main():
    ...
    print("\n" + "═"*68)
    print("  FRANCE MARKET — GOOGLE SHEETS EDITION  v1.0")
    ...
    print("📂 Loading France universe …"); universe = load_fr_universe()
    if universe.empty: print("❌ Empty universe — aborting."); return

    print(f"\n📡 Fetching index ({FR_INDEX}) …")
    ...
    if raw.empty: print("  ❌ Cannot fetch France index"); return
    cl = raw["Close"]
    if isinstance(cl, pd.DataFrame): cl=cl.squeeze()
    index_prices = _normalize(cl.dropna())
    print(f"  ✅ Index: {len(index_prices)} days")

    print("\n📡 France Sector ETFs …"); sector_prices = fetch_fr_sector_prices()
    ...
    run_time = datetime.now(tz_obj).strftime("%d %B %Y %H:%M CET")

    print("\n📸 Market Snapshot …");   snap_df    = build_fr_snapshot()
    ...
    print("📊 Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, FR_BREADTH_INDICES, INDEX_DATA_DIR, market="FR")
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "FR", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)

    # ── BUILD HTML FIRST (before Sheets writes that may hit quota) ──────────
    print("\n🌐 Building France HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "France_Market_Analysis.html")
        build_html_report(
    ...
    write_dashboard_tab(ss, dashboard_df, "FR");                  time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "FR");                        time.sleep(TAB_DELAY)
