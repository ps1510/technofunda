"""
FRANCE MARKET ANALYSIS — HTML-ONLY EDITION  v2.0
market_fr_gsht.py  — NO Google Sheets, generates FR.html only

Universe  : fr_cac40list.csv
Benchmark : ^FCHI
Timezone  : CET
Output    : FR.html
...
FR_INDEX          = "^FCHI"
FR_INDEX_FALLBACK = "^STOXX50E"

FR_INDUSTRY_TO_SECTOR = {
...
FR_BREADTH_INDICES = {
    "CAC 40":     {"yahoo": "^FCHI",     "csv": "fr_cac40list.csv"},
...
FR_SNAPSHOT_TICKERS = [
    ...
    {"name": "10Y OAT Yield",   "ticker": "^TNX",      "type": "Bond"},
...
# France sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
FR_SECTORS = {
...
def fetch_fr_sector_prices():
    ...
    for sec_name, cfg in FR_SECTORS.items():
    ...
    print(f"  ✅ France Sector prices: {len(result)}/{len(FR_SECTORS)}")
    return result
...
def build_fr_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in FR_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  FRANCE MARKET — HTML-ONLY  v2.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M CET')}")
    print("="*68+"\n")
    t0 = time.time()

    print(f"Loading France universe ..."); universe = load_fr_universe()
    if universe.empty: print("Empty universe — aborting."); return

    print(f"Fetching index ({FR_INDEX}) ...")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(FR_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  {FR_INDEX} empty, trying {FR_INDEX_FALLBACK} ...")
        raw = yf.download(FR_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  Cannot fetch France index"); return
    cl = raw["Close"]
    ...
    sector_prices = fetch_fr_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, FR_SECTORS)
    ...
    breadth_df = build_market_breadth(price_data, index_prices, FR_BREADTH_INDICES, INDEX_DATA_DIR, market="fr")
    ...
    stock_df = build_stock_strength(universe, price_data, index_prices, sector_prices, patterns_by_sym,
        market="fr", fetch_financials=FETCH_FINANCIALS,
        ohlcv_dict=ohlcv_dict if ENABLE_SIGNALS else {}, primary_rs=PRIMARY_RS_PERIOD)
    top_buy_df  = build_top_picks_buy(stock_df, sec_str_df, market="fr", primary_rs=PRIMARY_RS_PERIOD)
    top_sell_df = build_top_picks_sell(stock_df, sec_str_df, market="fr", primary_rs=PRIMARY_RS_PERIOD)
    chart_df    = build_chart_patterns_df(patterns_list, stock_df, market="fr")
    trade_df    = build_trade_setups(stock_df, sec_str_df, market="fr", primary_rs=PRIMARY_RS_PERIOD)
    sleeve_df   = build_rs_sleeve_list(stock_df, universe, INDEX_DATA_DIR, market="fr", run_time=run_time,
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "fr", run_time, primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="FR")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("Building France HTML report ...")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "FR.html")
        build_html_report(
            market="fr", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    print(f"\n{'='*68}")
    print(f"  COMPLETE!  |  {elapsed:.0f}s  |  FR.html")
    print("="*68)
