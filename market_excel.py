...
def build_workbook(market, snapshot_df, sector_str_df, sector_rot_df,
    ...
    # ── 4. Rotation (Sector + Industry) ──────────────────────────────────────
    # Combine into one sheet with a gap
    combined_rot = _combine_rotation(sector_rot_df, industry_rot_df)
    ws = wb.create_sheet("🔄 Rotation")
    write_sheet(ws, combined_rot, "🔄 Sector Rotation",
                "🔄  SECTOR & INDUSTRY ROTATION  —  Breadth, RS%, Zone Scores")

    # ── 5. Stocks (simplified main view) ─────────────────────────────────────
    ...
    # ── 13. Signal Detail (ALL raw columns) ───────────────────────────────────
