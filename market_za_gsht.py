"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  SOUTH AFRICA MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_za_gsht.py  — NO Google Sheets, generates ZA.html only       ║
║                                                                              ║
║  Universe  : za_jselist.csv                                                           ║
║  Benchmark : ^J200.JO                                                         ║
║  Timezone  : SAST                                                            ║
║  Output    : ZA.html                                                    ║
║                                                                              ║
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "za_all_stocks_master.csv")
...
# ─────────────────────────────────────────────────────────────────────────────
#  SOUTH AFRICA MARKET CONFIG
# ─────────────────────────────────────────────────────────────────────────────

ZA_INDEX          = "^J200.JO"
ZA_INDEX_FALLBACK = "^JN0U.JO"

ZA_INDUSTRY_TO_SECTOR = {
...
def load_za_universe():
    csv_path = STOCK_CSV  # full master list
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "za_jselist.csv")
    if not os.path.exists(csv_path):
...
# South Africa sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
ZA_SECTORS = {
...
def fetch_za_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in ZA_SECTORS.items():
    ...
    print(f"  ✅ South Africa Sector prices: {len(result)}/{len(ZA_SECTORS)}")
    return result
...
def build_za_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in ZA_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  SOUTH AFRICA MARKET — HTML-ONLY EDITION  v1.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M SAST')}")
    print(f"  Stocks:{MAX_STOCKS}  Patterns:{PATTERN_MAX}  Signals:{SIGNAL_MAX_STOCKS}  RS:{PRIMARY_RS_PERIOD}d")
    print("="*68+"\n")
    t0 = time.time()

    print(f"\U0001F4C2 Loading South Africa universe …"); universe = load_za_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({ZA_INDEX}) …")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(ZA_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  \u26a0 {ZA_INDEX} empty, trying {ZA_INDEX_FALLBACK} …")
        raw = yf.download(ZA_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  \u274c Cannot fetch South Africa index"); return
    cl = raw["Close"]
    ...
    print(f"\n\U0001F4E1 South Africa Sector ETFs …"); sector_prices = fetch_za_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, ZA_SECTORS)
    ...
    tz_obj   = timezone(timedelta(hours=2))
    run_time = datetime.now(tz_obj).strftime(f"%d %B %Y %H:%M SAST")

    print("\n\U0001F4F8 Market Snapshot …");   snap_df    = build_za_snapshot()
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, ZA_BREADTH_INDICES, INDEX_DATA_DIR, market="za")
    ...
    stock_df = build_stock_strength(
        ...
        market="za", fetch_financials=FETCH_FINANCIALS,
    ...
    print("\U0001F3C6 Top Picks – Buy …");    top_buy_df  = build_top_picks_buy(
        stock_df, sec_str_df, market="za", primary_rs=PRIMARY_RS_PERIOD)
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "za", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="ZA")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("\n\U0001F310 Building South Africa HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "ZA.html")
        build_html_report(
            market="za", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="za",
    ...
    print(f"\n{'='*68}")
    print(f"  \u2705  COMPLETE!  |  \u23f1 {elapsed:.0f}s  |  \U0001F4C4 ZA.html")
