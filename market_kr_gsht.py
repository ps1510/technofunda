"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  SOUTH KOREA MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                ║
║  market_kr_gsht.py  — GitHub Actions compatible                  ║
║                                                                            ║
║  Universe  : kr_kospilist.csv                                         ║
║  Benchmark : ^KS11                                                ║
║  Timezone  : KST                                                    ║
║  Sheet URL : GOOGLE_SHEET_URL_KR env var                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_KR")
    or "https://docs.google.com/spreadsheets/d/YOUR_KR_SHEET_ID/edit"
)
GSCOPE = [
...
def load_kr_universe():
    ...
    df["Sector"] = df["Sector"].map(KR_INDUSTRY_TO_SECTOR).fillna(df.get("Sector","General"))
...
def fetch_kr_sector_prices():
    """Fetch SOUTH KOREA sector ETF prices (returns empty dict — no local ETFs)."""
    sector_prices = {}
    for sec, cfg in KR_SECTORS.items():
        tkr = cfg.get("yahoo")
        if not tkr:
            continue
        try:
...
def main():
    ...
    print("\n" + "═"*68)
    print(f"  SOUTH KOREA MARKET — GOOGLE SHEETS EDITION  v1.0")
    ...
    print(f"\n📡 South Korea Sector ETFs …"); sector_prices = fetch_kr_sector_prices()
    ...
    print("\n📸 Market Snapshot …");   snap_df    = build_kr_snapshot()
    ...
    print("📊 Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, KR_BREADTH_INDICES, INDEX_DATA_DIR, market="KR")
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "KR", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)

    # ── BUILD HTML ──────────────────────────────────────────────────────────
    print("\n🌐 Building South Korea HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "SouthKorea_Market_Analysis.html")
        build_html_report(
    ...
    write_dashboard_tab(ss, dashboard_df, "KR");             time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "KR");                   time.sleep(TAB_DELAY)
