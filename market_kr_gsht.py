"""
SOUTH KOREA MARKET ANALYSIS — HTML-ONLY EDITION  v2.0
market_kr_gsht.py  — NO Google Sheets, generates KR.html only

Universe  : kr_kospilist.csv
Benchmark : ^KS11
Timezone  : KST
Output    : KR.html
...
KR_INDEX          = "^KS11"
KR_INDEX_FALLBACK = "^KS200"

KR_INDUSTRY_TO_SECTOR = {
...
KR_BREADTH_INDICES = {
    "KOSPI": {"yahoo": "^KS11", "csv": "kr_kospilist.csv"},
    "KOSDAQ": {"yahoo": "^KQ11", "csv": None},
}

KR_SNAPSHOT_TICKERS = [
    {"name": "KOSPI",                   "ticker": "^KS11",         "type": "Index"},
    {"name": "KOSDAQ",                  "ticker": "^KQ11",         "type": "Index"},
    {"name": "Nikkei 225",              "ticker": "^N225",         "type": "Index"},
    {"name": "KRW/USD",                 "ticker": "KRW=X",         "type": "Forex"},
    {"name": "USD/KRW",                 "ticker": "USDKRW=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y KTB Yield",           "ticker": "^TNX",          "type": "Bond"},
...
def load_kr_universe():
    ...
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "kr_kospilist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(KR_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# South Korea sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
KR_SECTORS = {
...
def fetch_kr_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in KR_SECTORS.items():
    ...
    print(f"  ✅ South Korea Sector prices: {len(result)}/{len(KR_SECTORS)}")
    return result
...
def build_kr_snapshot():
    ...
    syms = [t["ticker"] for t in KR_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  SOUTH KOREA MARKET — HTML-ONLY  v2.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M KST')}")
    print("="*68+"\n")
    t0 = time.time()

    print(f"Loading South Korea universe ..."); universe = load_kr_universe()
    if universe.empty: print("Empty universe — aborting."); return

    print(f"Fetching index ({KR_INDEX}) ...")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(KR_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  {KR_INDEX} empty, trying {KR_INDEX_FALLBACK} ...")
        raw = yf.download(KR_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  Cannot fetch South Korea index"); return
    cl = raw["Close"]
    ...
    sector_prices = fetch_kr_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, KR_SECTORS)
    ...
    run_time = datetime.now(tz_obj).strftime(f"%d %B %Y %H:%M KST")

    snap_df    = build_kr_snapshot()
    ...
    breadth_df = build_market_breadth(price_data, index_prices, KR_BREADTH_INDICES, INDEX_DATA_DIR, market="kr")
    ...
    stock_df = build_stock_strength(universe, price_data, index_prices, sector_prices, patterns_by_sym,
        market="kr", fetch_financials=FETCH_FINANCIALS,
        ohlcv_dict=ohlcv_dict if ENABLE_SIGNALS else {}, primary_rs=PRIMARY_RS_PERIOD)
    top_buy_df  = build_top_picks_buy(stock_df, sec_str_df, market="kr", primary_rs=PRIMARY_RS_PERIOD)
    top_sell_df = build_top_picks_sell(stock_df, sec_str_df, market="kr", primary_rs=PRIMARY_RS_PERIOD)
    chart_df    = build_chart_patterns_df(patterns_list, stock_df, market="kr")
    trade_df    = build_trade_setups(stock_df, sec_str_df, market="kr", primary_rs=PRIMARY_RS_PERIOD)
    sleeve_df   = build_rs_sleeve_list(stock_df, universe, INDEX_DATA_DIR, market="kr", run_time=run_time,
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "kr", run_time, primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="KR")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("Building South Korea HTML report ...")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "KR.html")
        build_html_report(
            market="kr", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="kr",
    ...
    print(f"\n{'='*68}")
    print(f"  COMPLETE!  |  {elapsed:.0f}s  |  KR.html")
    print("="*68)
