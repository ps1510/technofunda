"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  SPAIN MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                ║
║  market_es_gsht.py  — GitHub Actions compatible                  ║
║                                                                            ║
║  Universe  : es_ibexlist.csv                                         ║
║  Benchmark : ^IBEX                                                ║
║  Timezone  : CET                                                    ║
║  Sheet URL : GOOGLE_SHEET_URL_ES env var                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_ES")
    or "https://docs.google.com/spreadsheets/d/YOUR_ES_SHEET_ID/edit"
)
GSCOPE = [
...
def load_es_universe():
    ...
    df["Sector"] = df["Sector"].map(ES_INDUSTRY_TO_SECTOR).fillna(df.get("Sector","General"))
...
def fetch_es_sector_prices():
    """Fetch SPAIN sector ETF prices (returns empty dict — no local ETFs)."""
    sector_prices = {}
    for sec, cfg in ES_SECTORS.items():
        tkr = cfg.get("yahoo")
        if not tkr:
            continue
        try:
...
def main():
    ...
    print("\n" + "═"*68)
    print(f"  SPAIN MARKET — GOOGLE SHEETS EDITION  v1.0")
    ...
    print(f"\n📡 Spain Sector ETFs …"); sector_prices = fetch_es_sector_prices()
    ...
    print("\n📸 Market Snapshot …");   snap_df    = build_es_snapshot()
    ...
    print("📊 Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, ES_BREADTH_INDICES, INDEX_DATA_DIR, market="ES")
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "ES", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)

    # ── BUILD HTML ──────────────────────────────────────────────────────────
    print("\n🌐 Building Spain HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "Spain_Market_Analysis.html")
        build_html_report(
    ...
    write_dashboard_tab(ss, dashboard_df, "ES");             time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "ES");                   time.sleep(TAB_DELAY)
