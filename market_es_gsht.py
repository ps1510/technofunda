"""
SPAIN MARKET ANALYSIS — HTML-ONLY EDITION  v2.0
market_es_gsht.py  — NO Google Sheets, generates ES.html only

Universe  : es_ibexlist.csv
Benchmark : ^IBEX
Timezone  : CET
Output    : ES.html
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "es_all_stocks_master.csv")
...
ES_INDEX          = "^IBEX"
ES_INDEX_FALLBACK = "^STOXX50E"

ES_INDUSTRY_TO_SECTOR = {
...
ES_BREADTH_INDICES = {
    "IBEX 35": {"yahoo": "^IBEX", "csv": "es_ibexlist.csv"},
    "Euro Stoxx": {"yahoo": "^STOXX50E", "csv": None},
}

ES_SNAPSHOT_TICKERS = [
    {"name": "IBEX 35",                 "ticker": "^IBEX",         "type": "Index"},
    ...
    {"name": "10Y Bonos Yield",         "ticker": "^TNX",          "type": "Bond"},
...
# Spain sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
ES_SECTORS = {
...
def fetch_es_sector_prices():
    ...
    for sec_name, cfg in ES_SECTORS.items():
    ...
    print(f"  ✅ Spain Sector prices: {len(result)}/{len(ES_SECTORS)}")
    return result
...
def build_es_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in ES_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  SPAIN MARKET — HTML-ONLY  v2.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M CET')}")
    print("="*68+"\n")
    t0 = time.time()

    print(f"Loading Spain universe ..."); universe = load_es_universe()
    if universe.empty: print("Empty universe — aborting."); return

    print(f"Fetching index ({ES_INDEX}) ...")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(ES_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  {ES_INDEX} empty, trying {ES_INDEX_FALLBACK} ...")
        raw = yf.download(ES_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  Cannot fetch Spain index"); return
    cl = raw["Close"]
    ...
    sector_prices = fetch_es_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, ES_SECTORS)
    ...
    breadth_df = build_market_breadth(price_data, index_prices, ES_BREADTH_INDICES, INDEX_DATA_DIR, market="es")
    ...
    stock_df = build_stock_strength(universe, price_data, index_prices, sector_prices, patterns_by_sym,
        market="es", fetch_financials=FETCH_FINANCIALS,
        ohlcv_dict=ohlcv_dict if ENABLE_SIGNALS else {}, primary_rs=PRIMARY_RS_PERIOD)
    top_buy_df  = build_top_picks_buy(stock_df, sec_str_df, market="es", primary_rs=PRIMARY_RS_PERIOD)
    top_sell_df = build_top_picks_sell(stock_df, sec_str_df, market="es", primary_rs=PRIMARY_RS_PERIOD)
    chart_df    = build_chart_patterns_df(patterns_list, stock_df, market="es")
    trade_df    = build_trade_setups(stock_df, sec_str_df, market="es", primary_rs=PRIMARY_RS_PERIOD)
    sleeve_df   = build_rs_sleeve_list(stock_df, universe, INDEX_DATA_DIR, market="es", run_time=run_time,
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "es", run_time, primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="ES")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("Building Spain HTML report ...")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "ES.html")
        build_html_report(
            market="es", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="es",
    ...
    print(f"\n{'='*68}")
    print(f"  COMPLETE!  |  {elapsed:.0f}s  |  ES.html")
    print("="*68)
