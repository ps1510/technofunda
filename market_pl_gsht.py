"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  POLAND MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_pl_gsht.py  — NO Google Sheets, generates PL.html only       ║
║                                                                              ║
║  Universe  : pl_wig20list.csv                                                           ║
║  Benchmark : WIG20.WA                                                         ║
║  Timezone  : CET                                                            ║
║  Output    : PL.html                                                    ║
║                                                                              ║
...
PL_BREADTH_INDICES = {
    "WIG20": {"yahoo": "WIG20.WA", "csv": "pl_wig20list.csv"},
}

PL_SNAPSHOT_TICKERS = [
    {"name": "WIG20",                   "ticker": "WIG20.WA",      "type": "Index"},
    {"name": "Euro Stoxx 50",           "ticker": "^STOXX50E",     "type": "Index"},
    {"name": "DAX 40",                  "ticker": "^GDAXI",        "type": "Index"},
    {"name": "PLN/USD",                 "ticker": "PLN=X",         "type": "Forex"},
    {"name": "EUR/PLN",                 "ticker": "EURPLN=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y PolGB Yield",         "ticker": "^TNX",          "type": "Bond"},
...
PL_SECTORS = {
    ...
    "Health Care":      {"yahoo": None, "csv": "pl_sector_health_care.csv"},
    "Industrials":     {"yahoo": None, "csv": "pl_sector_industrials.csv"},
    "Consumer Discretionary":    {"yahoo": None, "csv": "pl_sector_consumer_discretionary.csv"},
    "Consumer Staples":{"yahoo": None, "csv": "pl_sector_consumer_staples.csv"},
    "Utilities":       {"yahoo": None, "csv": "pl_sector_utilities.csv"},
    "Communication Services":    {"yahoo": None, "csv": "pl_sector_communication_services.csv"},
    "Real Estate":      {"yahoo": None, "csv": "pl_sector_real_estate.csv"},
}
...
def build_pl_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in PL_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  POLAND MARKET — HTML-ONLY EDITION  v1.0")
    ...
    print(f"\U0001F4C2 Loading Poland universe …"); universe = load_pl_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({PL_INDEX}) …")
    ...
    print(f"\n\U0001F4E1 Poland Sector ETFs …"); sector_prices = fetch_pl_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, PL_SECTORS)
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, PL_BREADTH_INDICES, INDEX_DATA_DIR, market="pl")
    ...
    stock_df = build_stock_strength(
        ...
        market="pl", fetch_financials=FETCH_FINANCIALS,
    ...
    print("\U0001F3C6 Top Picks – Buy …");    top_buy_df  = build_top_picks_buy(
        stock_df, sec_str_df, market="pl", primary_rs=PRIMARY_RS_PERIOD)
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "pl", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="PL")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("\n\U0001F310 Building Poland HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "PL.html")
        build_html_report(
            market="pl", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="pl",
    ...
    print(f"\n{'='*68}")
    print(f"  \u2705  COMPLETE!  |  \u23f1 {elapsed:.0f}s  |  \U0001F4C4 PL.html")
