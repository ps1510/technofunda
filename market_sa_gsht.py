"""
SAUDI ARABIA MARKET ANALYSIS — HTML-ONLY EDITION  v2.0
market_sa_gsht.py  — NO Google Sheets, generates SA.html only

Universe  : sa_tadawullist.csv
Benchmark : ^TASI.SR
Timezone  : AST
Output    : SA.html
...
SA_BREADTH_INDICES = {
    "TASI": {"yahoo": "^TASI.SR", "csv": "sa_tadawullist.csv"},
}

SA_SNAPSHOT_TICKERS = [
    {"name": "TASI",                    "ticker": "^TASI.SR",      "type": "Index"},
    {"name": "UAE DFM",                 "ticker": "^DFMGI",        "type": "Index"},
    {"name": "Kuwait All-Share",        "ticker": "^KWSE",         "type": "Index"},
    {"name": "SAR/USD",                 "ticker": "SAR=X",         "type": "Forex"},
    {"name": "USD/SAR",                 "ticker": "USDSAR=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y Saudi Yield",         "ticker": "^TNX",          "type": "Bond"},
...
# Saudi Arabia sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
SA_SECTORS = {
...
def fetch_sa_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in SA_SECTORS.items():
    ...
    print(f"  ✅ Saudi Arabia Sector prices: {len(result)}/{len(SA_SECTORS)}")
    return result
...
def build_sa_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in SA_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  SAUDI ARABIA MARKET — HTML-ONLY  v2.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M AST')}")
    print("="*68+"\n")
    t0 = time.time()

    print(f"Loading Saudi Arabia universe ..."); universe = load_sa_universe()
    if universe.empty: print("Empty universe — aborting."); return

    print(f"Fetching index ({SA_INDEX}) ...")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(SA_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  {SA_INDEX} empty, trying {SA_INDEX_FALLBACK} ...")
        raw = yf.download(SA_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  Cannot fetch Saudi Arabia index"); return
    cl = raw["Close"]
    ...
    sector_prices = fetch_sa_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, SA_SECTORS)
    ...
    run_time = datetime.now(tz_obj).strftime(f"%d %B %Y %H:%M AST")

    snap_df    = build_sa_snapshot()
    ...
    breadth_df = build_market_breadth(price_data, index_prices, SA_BREADTH_INDICES, INDEX_DATA_DIR, market="sa")
    ...
    stock_df = build_stock_strength(universe, price_data, index_prices, sector_prices, patterns_by_sym,
        market="sa", fetch_financials=FETCH_FINANCIALS,
        ohlcv_dict=ohlcv_dict if ENABLE_SIGNALS else {}, primary_rs=PRIMARY_RS_PERIOD)
    top_buy_df  = build_top_picks_buy(stock_df, sec_str_df, market="sa", primary_rs=PRIMARY_RS_PERIOD)
    top_sell_df = build_top_picks_sell(stock_df, sec_str_df, market="sa", primary_rs=PRIMARY_RS_PERIOD)
    chart_df    = build_chart_patterns_df(patterns_list, stock_df, market="sa")
    trade_df    = build_trade_setups(stock_df, sec_str_df, market="sa", primary_rs=PRIMARY_RS_PERIOD)
    sleeve_df   = build_rs_sleeve_list(stock_df, universe, INDEX_DATA_DIR, market="sa", run_time=run_time,
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "sa", run_time, primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="SA")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("Building Saudi Arabia HTML report ...")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "SA.html")
        build_html_report(
            market="sa", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    print(f"\n{'='*68}")
    print(f"  COMPLETE!  |  {elapsed:.0f}s  |  SA.html")
    print("="*68)
