"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  SAUDI ARABIA MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                ║
║  market_sa_gsht.py  — GitHub Actions compatible                  ║
║                                                                            ║
║  Universe  : sa_tadawullist.csv                                         ║
║  Benchmark : ^TASI.SR                                                ║
║  Timezone  : AST                                                    ║
║  Sheet URL : GOOGLE_SHEET_URL_SA env var                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_SA")
    or "https://docs.google.com/spreadsheets/d/YOUR_SA_SHEET_ID/edit"
)
GSCOPE = [
...
def fetch_sa_sector_prices():
    """Fetch SAUDI ARABIA sector ETF prices (returns empty dict — no local ETFs)."""
    sector_prices = {}
    for sec, cfg in SA_SECTORS.items():
        tkr = cfg.get("yahoo")
        if not tkr:
            continue
        try:
...
def main():
    ...
    print("\n" + "═"*68)
    print(f"  SAUDI ARABIA MARKET — GOOGLE SHEETS EDITION  v1.0")
    ...
    print(f"\n📡 Saudi Arabia Sector ETFs …"); sector_prices = fetch_sa_sector_prices()
    ...
    print("\n📸 Market Snapshot …");   snap_df    = build_sa_snapshot()
    ...
    print("📊 Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, SA_BREADTH_INDICES, INDEX_DATA_DIR, market="SA")
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "SA", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)

    # ── BUILD HTML ──────────────────────────────────────────────────────────
    print("\n🌐 Building Saudi Arabia HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "SaudiArabia_Market_Analysis.html")
        build_html_report(
    ...
    write_dashboard_tab(ss, dashboard_df, "SA");             time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "SA");                   time.sleep(TAB_DELAY)
