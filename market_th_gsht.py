"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  THAILAND MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_th_gsht.py  — NO Google Sheets, generates TH.html only       ║
║                                                                              ║
║  Universe  : th_setlist.csv                                                           ║
║  Benchmark : ^SET.BK                                                         ║
║  Timezone  : ICT                                                            ║
║  Output    : TH.html                                                    ║
║                                                                              ║
...
# ─────────────────────────────────────────────────────────────────────────────
#  THAILAND MARKET CONFIG
# ─────────────────────────────────────────────────────────────────────────────

TH_INDEX          = "^SET.BK"
TH_INDEX_FALLBACK = "^SETI"

TH_INDUSTRY_TO_SECTOR = {
...
TH_BREADTH_INDICES = {
    "SET Index": {"yahoo": "^SET.BK", "csv": "th_setlist.csv"},
}

TH_SNAPSHOT_TICKERS = [
    {"name": "SET Index",               "ticker": "^SET.BK",       "type": "Index"},
    {"name": "Straits Times",           "ticker": "^STI",          "type": "Index"},
    {"name": "Hang Seng",               "ticker": "^HSI",          "type": "Index"},
    {"name": "THB/USD",                 "ticker": "THB=X",         "type": "Forex"},
    {"name": "USD/THB",                 "ticker": "USDTHB=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y ThaiGB Yield",        "ticker": "^TNX",          "type": "Bond"},
...
def load_th_universe():
    csv_path = STOCK_CSV  # full master list
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "th_setlist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(TH_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# Thailand sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
TH_SECTORS = {
...
def fetch_th_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in TH_SECTORS.items():
    ...
    print(f"  ✅ Thailand Sector prices: {len(result)}/{len(TH_SECTORS)}")
    return result
...
def build_th_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in TH_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  THAILAND MARKET — HTML-ONLY EDITION  v1.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M ICT')}")
    print(f"  Stocks:{MAX_STOCKS}  Patterns:{PATTERN_MAX}  Signals:{SIGNAL_MAX_STOCKS}  RS:{PRIMARY_RS_PERIOD}d")
    print("="*68+"\n")
    t0 = time.time()

    print(f"\U0001F4C2 Loading Thailand universe …"); universe = load_th_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({TH_INDEX}) …")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(TH_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  \u26a0 {TH_INDEX} empty, trying {TH_INDEX_FALLBACK} …")
        raw = yf.download(TH_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  \u274c Cannot fetch Thailand index"); return
    cl = raw["Close"]
    ...
    print(f"\n\U0001F4E1 Thailand Sector ETFs …"); sector_prices = fetch_th_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, TH_SECTORS)
    ...
    run_time = datetime.now(tz_obj).strftime(f"%d %B %Y %H:%M ICT")

    print("\n\U0001F4F8 Market Snapshot …");   snap_df    = build_th_snapshot()
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, TH_BREADTH_INDICES, INDEX_DATA_DIR, market="th")
    ...
    stock_df = build_stock_strength(
        ...
        market="th", fetch_financials=FETCH_FINANCIALS,
    ...
    print("\U0001F3C6 Top Picks – Buy …");    top_buy_df  = build_top_picks_buy(
        stock_df, sec_str_df, market="th", primary_rs=PRIMARY_RS_PERIOD)
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "th", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="TH")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("\n\U0001F310 Building Thailand HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "TH.html")
        build_html_report(
            market="th", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="th",
    ...
    print(f"\n{'='*68}")
    print(f"  \u2705  COMPLETE!  |  \u23f1 {elapsed:.0f}s  |  \U0001F4C4 TH.html")
