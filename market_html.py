...
def _build_table(df, table_id, searchable=True, max_rows=2000):
    ...
    search = (f'<div class="tbl-search"><input type="text" placeholder="🔍 Filter…"'
              f' oninput="filterTable(this,\'{table_id}\')"></div>') if searchable else ""
...
def _build_opportunity_cards(df):
  ...
  <div class="opp-head"><span class="opp-sym">{sym}</span><span class="sl-badge {sl_c}">{sl}</span></div>
  ...
  Risk% is shown as reference only.</span>
    ...
    for key, rows in sections.items():
        ...
        for _, row in df_sec.iterrows():
            ...
            tds = ""
            for c in show_cols:
                val=row.get(c,""); display=_fmt(val); cls=_cell_class(c,val)
                align="left" if c.lower() in _LEFT_COLS else "center"
                ca=f' class="{cls}"' if cls else ""
                tds += f'<td{ca} style="text-align:{align}">{display}</td>'
