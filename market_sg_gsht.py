"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  SINGAPORE MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_sg_gsht.py  — NO Google Sheets, generates SG.html only       ║
║                                                                              ║
║  Universe  : sg_stilist.csv                                                           ║
║  Benchmark : ^STI                                                         ║
║  Timezone  : SGT                                                            ║
║  Output    : SG.html                                                    ║
║                                                                              ║
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "sg_all_stocks_master.csv")
...
# ─────────────────────────────────────────────────────────────────────────────
#  SINGAPORE MARKET CONFIG
# ─────────────────────────────────────────────────────────────────────────────

SG_INDEX          = "^STI"
SG_INDEX_FALLBACK = "ES3.SI"

SG_INDUSTRY_TO_SECTOR = {
...
SG_BREADTH_INDICES = {
    "STI": {"yahoo": "^STI", "csv": "sg_stilist.csv"},
}
...
SG_SNAPSHOT_TICKERS = [
    ...
    {"name": "SGD/USD",                 "ticker": "SGD=X",         "type": "Forex"},
    {"name": "USD/SGD",                 "ticker": "USDSGD=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y SGS Yield",           "ticker": "^TNX",          "type": "Bond"},
...
def load_sg_universe():
    ...
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "sg_stilist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(SG_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# Singapore sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
SG_SECTORS = {
...
def fetch_sg_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in SG_SECTORS.items():
    ...
    print(f"  ✅ Singapore Sector prices: {len(result)}/{len(SG_SECTORS)}")
    return result
...
def build_sg_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in SG_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  SINGAPORE MARKET — HTML-ONLY EDITION  v1.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M SGT')}")
    print(f"  Stocks:{MAX_STOCKS}  Patterns:{PATTERN_MAX}  Signals:{SIGNAL_MAX_STOCKS}  RS:{PRIMARY_RS_PERIOD}d")
    print("="*68+"\n")
    t0 = time.time()

    print(f"\U0001F4C2 Loading Singapore universe …"); universe = load_sg_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({SG_INDEX}) …")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(SG_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  \u26a0 {SG_INDEX} empty, trying {SG_INDEX_FALLBACK} …")
        raw = yf.download(SG_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  \u274c Cannot fetch Singapore index"); return
    cl = raw["Close"]
    ...
    print(f"\n\U0001F4E1 Singapore Sector ETFs …"); sector_prices = fetch_sg_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, SG_SECTORS)
    ...
    run_time = datetime.now(tz_obj).strftime(f"%d %B %Y %H:%M SGT")

    print("\n\U0001F4F8 Market Snapshot …");   snap_df    = build_sg_snapshot()
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, SG_BREADTH_INDICES, INDEX_DATA_DIR, market="sg")
    ...
    stock_df = build_stock_strength(
        ...
        market="sg", fetch_financials=FETCH_FINANCIALS,
    ...
    print("\U0001F3C6 Top Picks – Buy …");    top_buy_df  = build_top_picks_buy(
        stock_df, sec_str_df, market="sg", primary_rs=PRIMARY_RS_PERIOD)
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "sg", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="SG")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("\n\U0001F310 Building Singapore HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "SG.html")
        build_html_report(
            market="sg", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="sg",
    ...
    print(f"\n{'='*68}")
    print(f"  \u2705  COMPLETE!  |  \u23f1 {elapsed:.0f}s  |  \U0001F4C4 SG.html")
