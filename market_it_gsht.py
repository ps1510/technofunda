"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  ITALY MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_it_gsht.py  — NO Google Sheets, generates IT.html only       ║
║                                                                              ║
║  Universe  : it_ftsemiblist.csv                                                           ║
...
IT_INDEX          = "FTSEMIB.MI"
IT_INDEX_FALLBACK = "^STOXX50E"

IT_INDUSTRY_TO_SECTOR = {
...
IT_BREADTH_INDICES = {
    "FTSE MIB": {"yahoo": "FTSEMIB.MI", "csv": "it_ftsemiblist.csv"},
}
...
IT_SNAPSHOT_TICKERS = [
    ...
    {"name": "10Y BTP Yield",           "ticker": "^TNX",          "type": "Bond"},
...
def load_it_universe():
    ...
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "it_ftsemiblist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(IT_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
def build_it_snapshot():
    ...
    syms = [t["ticker"] for t in IT_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  ITALY MARKET — HTML-ONLY EDITION  v1.0")
    ...
    print(f"\U0001F4C2 Loading Italy universe …"); universe = load_it_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({IT_INDEX}) …")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(IT_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  \u26a0 {IT_INDEX} empty, trying {IT_INDEX_FALLBACK} …")
        raw = yf.download(IT_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  \u274c Cannot fetch Italy index"); return
    cl = raw["Close"]
    ...
    print(f"\n\U0001F4E1 Italy Sector ETFs …"); sector_prices = fetch_it_sector_prices()
    ...
    print("\n\U0001F4F8 Market Snapshot …");   snap_df    = build_it_snapshot()
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, IT_BREADTH_INDICES, INDEX_DATA_DIR, market="it")
    ...
    stock_df = build_stock_strength(
        ...
        market="it", fetch_financials=FETCH_FINANCIALS,
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="IT")
