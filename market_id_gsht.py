"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  INDONESIA MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_id_gsht.py  — NO Google Sheets, generates ID.html only       ║
║                                                                              ║
║  Universe  : id_idxlist.csv                                                           ║
║  Benchmark : ^JKSE                                                         ║
║  Timezone  : WIB                                                            ║
║  Output    : ID.html                                                    ║
║                                                                              ║
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "id_all_stocks_master.csv")
...
ID_INDEX          = "^JKSE"
ID_INDEX_FALLBACK = "^JKLQ45"

ID_INDUSTRY_TO_SECTOR = {
...
ID_BREADTH_INDICES = {
    "IDX Composite": {"yahoo": "^JKSE", "csv": "id_idxlist.csv"},
}

ID_SNAPSHOT_TICKERS = [
    {"name": "IDX Composite",           "ticker": "^JKSE",         "type": "Index"},
    {"name": "LQ45",                    "ticker": "^JKLQ45",       "type": "Index"},
    {"name": "Straits Times",           "ticker": "^STI",          "type": "Index"},
    {"name": "IDR/USD",                 "ticker": "IDR=X",         "type": "Forex"},
    {"name": "USD/IDR",                 "ticker": "USDIDR=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y INDOGB Yield",        "ticker": "^TNX",          "type": "Bond"},
...
def load_id_universe():
    csv_path = STOCK_CSV  # full master list
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "id_idxlist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(ID_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# Indonesia sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
ID_SECTORS = {
...
def fetch_id_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in ID_SECTORS.items():
    ...
    print(f"  ✅ Indonesia Sector prices: {len(result)}/{len(ID_SECTORS)}")
    return result
...
def build_id_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in ID_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  INDONESIA MARKET — HTML-ONLY EDITION  v1.0")
    ...
    print(f"\U0001F4C2 Loading Indonesia universe …"); universe = load_id_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({ID_INDEX}) …")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(ID_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  \u26a0 {ID_INDEX} empty, trying {ID_INDEX_FALLBACK} …")
        raw = yf.download(ID_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  \u274c Cannot fetch Indonesia index"); return
    cl = raw["Close"]
    ...
    print(f"\n\U0001F4E1 Indonesia Sector ETFs …"); sector_prices = fetch_id_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, ID_SECTORS)
    ...
    tz_obj   = timezone(timedelta(hours=7))
    run_time = datetime.now(tz_obj).strftime(f"%d %B %Y %H:%M WIB")

    print("\n\U0001F4F8 Market Snapshot …");   snap_df    = build_id_snapshot()
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, ID_BREADTH_INDICES, INDEX_DATA_DIR, market="id")
    ...
    stock_df = build_stock_strength(
        ...
        market="id", fetch_financials=FETCH_FINANCIALS,
    ...
    print("\U0001F3C6 Top Picks – Buy …");    top_buy_df  = build_top_picks_buy(
        stock_df, sec_str_df, market="id", primary_rs=PRIMARY_RS_PERIOD)
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "id", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="ID")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("\n\U0001F310 Building Indonesia HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "ID.html")
        build_html_report(
            market="id", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="id",
    ...
    print(f"\n{'='*68}")
    print(f"  \u2705  COMPLETE!  |  \u23f1 {elapsed:.0f}s  |  \U0001F4C4 ID.html")
