"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  NETHERLANDS MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                ║
║  market_nl_gsht.py  — GitHub Actions compatible                  ║
║                                                                            ║
║  Universe  : nl_aexlist.csv                                         ║
║  Benchmark : ^AEX                                                ║
║  Timezone  : CET                                                    ║
║  Sheet URL : GOOGLE_SHEET_URL_NL env var                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_NL")
    or "https://docs.google.com/spreadsheets/d/YOUR_NL_SHEET_ID/edit"
)
GSCOPE = [
...
def load_nl_universe():
    ...
    df["Sector"] = df["Sector"].map(NL_INDUSTRY_TO_SECTOR).fillna(df.get("Sector","General"))
...
def fetch_nl_sector_prices():
    """Fetch NETHERLANDS sector ETF prices (returns empty dict — no local ETFs)."""
    sector_prices = {}
    for sec, cfg in NL_SECTORS.items():
        tkr = cfg.get("yahoo")
        if not tkr:
            continue
        try:
...
def main():
    ...
    print("\n" + "═"*68)
    print(f"  NETHERLANDS MARKET — GOOGLE SHEETS EDITION  v1.0")
    ...
    print(f"\n📡 Netherlands Sector ETFs …"); sector_prices = fetch_nl_sector_prices()
    ...
    print("\n📸 Market Snapshot …");   snap_df    = build_nl_snapshot()
    ...
    print("📊 Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, NL_BREADTH_INDICES, INDEX_DATA_DIR, market="NL")
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "NL", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)

    # ── BUILD HTML ──────────────────────────────────────────────────────────
    print("\n🌐 Building Netherlands HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "Netherlands_Market_Analysis.html")
        build_html_report(
    ...
    write_dashboard_tab(ss, dashboard_df, "NL");             time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "NL");                   time.sleep(TAB_DELAY)
