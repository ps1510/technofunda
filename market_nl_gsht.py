"""
NETHERLANDS MARKET ANALYSIS — HTML-ONLY EDITION  v2.0
market_nl_gsht.py  — NO Google Sheets, generates NL.html only

Universe  : nl_aexlist.csv
Benchmark : ^AEX
Timezone  : CET
Output    : NL.html
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "nl_all_stocks_master.csv")
...
NL_INDEX          = "^AEX"
NL_INDEX_FALLBACK = "^STOXX50E"

NL_INDUSTRY_TO_SECTOR = {
...
NL_BREADTH_INDICES = {
    "AEX": {"yahoo": "^AEX", "csv": "nl_aexlist.csv"},
    "Euro Stoxx": {"yahoo": "^STOXX50E", "csv": None},
}

NL_SNAPSHOT_TICKERS = [
    {"name": "AEX",                     "ticker": "^AEX",          "type": "Index"},
    ...
    {"name": "10Y Bund Yield",          "ticker": "^TNX",          "type": "Bond"},
...
def load_nl_universe():
    ...
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "nl_aexlist.csv")
    if not os.path.exists(csv_path):
...
# Netherlands sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
NL_SECTORS = {
...
def fetch_nl_sector_prices():
    ...
    for sec_name, cfg in NL_SECTORS.items():
    ...
    print(f"  ✅ Netherlands Sector prices: {len(result)}/{len(NL_SECTORS)}")
    return result
...
def build_nl_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in NL_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  NETHERLANDS MARKET — HTML-ONLY  v2.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M CET')}")
    print("="*68+"\n")
    t0 = time.time()

    print(f"Loading Netherlands universe ..."); universe = load_nl_universe()
    if universe.empty: print("Empty universe — aborting."); return

    print(f"Fetching index ({NL_INDEX}) ...")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(NL_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  {NL_INDEX} empty, trying {NL_INDEX_FALLBACK} ...")
        raw = yf.download(NL_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  Cannot fetch Netherlands index"); return
    cl = raw["Close"]
    ...
    sector_prices = fetch_nl_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, NL_SECTORS)
    ...
    breadth_df = build_market_breadth(price_data, index_prices, NL_BREADTH_INDICES, INDEX_DATA_DIR, market="nl")
    ...
    stock_df = build_stock_strength(universe, price_data, index_prices, sector_prices, patterns_by_sym,
        market="nl", fetch_financials=FETCH_FINANCIALS,
        ohlcv_dict=ohlcv_dict if ENABLE_SIGNALS else {}, primary_rs=PRIMARY_RS_PERIOD)
    top_buy_df  = build_top_picks_buy(stock_df, sec_str_df, market="nl", primary_rs=PRIMARY_RS_PERIOD)
    top_sell_df = build_top_picks_sell(stock_df, sec_str_df, market="nl", primary_rs=PRIMARY_RS_PERIOD)
    chart_df    = build_chart_patterns_df(patterns_list, stock_df, market="nl")
    trade_df    = build_trade_setups(stock_df, sec_str_df, market="nl", primary_rs=PRIMARY_RS_PERIOD)
    sleeve_df   = build_rs_sleeve_list(stock_df, universe, INDEX_DATA_DIR, market="nl", run_time=run_time,
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "nl", run_time, primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="NL")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("Building Netherlands HTML report ...")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "NL.html")
        build_html_report(
            market="nl", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="nl",
    ...
    print(f"\n{'='*68}")
    print(f"  COMPLETE!  |  {elapsed:.0f}s  |  NL.html")
    print("="*68)
