"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  SWITZERLAND MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                ║
║  market_ch_gsht.py  — GitHub Actions compatible                  ║
║                                                                            ║
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "ch_smilist.csv")
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_CH")
    or "https://docs.google.com/spreadsheets/d/YOUR_CH_SHEET_ID/edit"
)
GSCOPE = [
...
def load_ch_universe():
    ...
    df["Sector"] = df["Sector"].map(CH_INDUSTRY_TO_SECTOR).fillna(df.get("Sector","General"))
...
def fetch_ch_sector_prices():
    """Fetch SWITZERLAND sector ETF prices (returns empty dict — no local ETFs)."""
    sector_prices = {}
    for sec, cfg in CH_SECTORS.items():
        tkr = cfg.get("yahoo")
        if not tkr:
            continue
        try:
...
def main():
    ...
    print("\n" + "═"*68)
    print(f"  SWITZERLAND MARKET — GOOGLE SHEETS EDITION  v1.0")
    ...
    print(f"\n📡 Switzerland Sector ETFs …"); sector_prices = fetch_ch_sector_prices()
    ...
    print("\n📸 Market Snapshot …");   snap_df    = build_ch_snapshot()
    ...
    print("📊 Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, CH_BREADTH_INDICES, INDEX_DATA_DIR, market="CH")
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "CH", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)

    # ── BUILD HTML ──────────────────────────────────────────────────────────
    print("\n🌐 Building Switzerland HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "Switzerland_Market_Analysis.html")
        build_html_report(
    ...
    write_dashboard_tab(ss, dashboard_df, "CH");             time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "CH");                   time.sleep(TAB_DELAY)
