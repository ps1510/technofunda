"""
SWEDEN MARKET ANALYSIS — HTML-ONLY EDITION  v2.0
market_se_gsht.py  — NO Google Sheets, generates SE.html only

Universe  : se_omxlist.csv
Benchmark : ^OMX
Timezone  : CET
Output    : SE.html
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "se_all_stocks_master.csv")
...
SE_INDEX          = "^OMX"
SE_INDEX_FALLBACK = "^OMXSPI"

SE_INDUSTRY_TO_SECTOR = {
...
SE_BREADTH_INDICES = {
    "OMX S30": {"yahoo": "^OMX", "csv": "se_omxlist.csv"},
    "Euro Stoxx": {"yahoo": "^STOXX50E", "csv": None},
}

SE_SNAPSHOT_TICKERS = [
    {"name": "OMX Stockholm 30",        "ticker": "^OMX",          "type": "Index"},
    {"name": "OMX SPI",                 "ticker": "^OMXSPI",       "type": "Index"},
    {"name": "DAX 40",                  "ticker": "^GDAXI",        "type": "Index"},
    {"name": "SEK/USD",                 "ticker": "SEK=X",         "type": "Forex"},
    {"name": "EUR/SEK",                 "ticker": "EURSEK=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y Riksbank Yld",        "ticker": "^TNX",          "type": "Bond"},
...
def load_se_universe():
    ...
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "se_omxlist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(SE_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# Sweden sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
SE_SECTORS = {
...
def fetch_se_sector_prices():
    result = {}
    end   = __import__('datetime').datetime.today() + __import__('datetime').timedelta(days=1)
    start = end - __import__('datetime').timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in SE_SECTORS.items():
        ticker = cfg.get("yahoo")
        if not ticker: continue
        try:
            import yfinance as _yf
            raw = _yf.download(ticker, start=start.strftime("%Y-%m-%d"),
                              end=end.strftime("%Y-%m-%d"),
                              auto_adjust=True, progress=False)
            if raw.empty: continue
            cl = raw["Close"]
            if isinstance(cl, __import__('pandas').DataFrame): cl = cl.squeeze()
            s = _normalize(cl.dropna())
            if len(s) >= 22: result[sec_name] = s
        except Exception: pass
    print(f"  ✅ Sweden Sector prices: {len(result)}/{len(SE_SECTORS)}")
    return result
...
def build_se_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in SE_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  SWEDEN MARKET — HTML-ONLY  v2.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M CET')}")
    print("="*68+"\n")
    t0 = time.time()

    print(f"Loading Sweden universe ..."); universe = load_se_universe()
    if universe.empty: print("Empty universe — aborting."); return

    print(f"Fetching index ({SE_INDEX}) ...")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(SE_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  {SE_INDEX} empty, trying {SE_INDEX_FALLBACK} ...")
        raw = yf.download(SE_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  Cannot fetch Sweden index"); return
    cl = raw["Close"]
    ...
    sector_prices = fetch_se_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, SE_SECTORS)
    ...
    breadth_df = build_market_breadth(price_data, index_prices, SE_BREADTH_INDICES, INDEX_DATA_DIR, market="se")
    ...
    stock_df = build_stock_strength(universe, price_data, index_prices, sector_prices, patterns_by_sym,
        market="se", fetch_financials=FETCH_FINANCIALS,
        ohlcv_dict=ohlcv_dict if ENABLE_SIGNALS else {}, primary_rs=PRIMARY_RS_PERIOD)
    top_buy_df  = build_top_picks_buy(stock_df, sec_str_df, market="se", primary_rs=PRIMARY_RS_PERIOD)
    top_sell_df = build_top_picks_sell(stock_df, sec_str_df, market="se", primary_rs=PRIMARY_RS_PERIOD)
    chart_df    = build_chart_patterns_df(patterns_list, stock_df, market="se")
    trade_df    = build_trade_setups(stock_df, sec_str_df, market="se", primary_rs=PRIMARY_RS_PERIOD)
    sleeve_df   = build_rs_sleeve_list(stock_df, universe, INDEX_DATA_DIR, market="se", run_time=run_time,
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "se", run_time, primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="SE")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("Building Sweden HTML report ...")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "SE.html")
        build_html_report(
            market="se", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="se",
    ...
    print(f"\n{'='*68}")
    print(f"  COMPLETE!  |  {elapsed:.0f}s  |  SE.html")
    print("="*68)
