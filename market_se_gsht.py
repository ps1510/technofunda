"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  SWEDEN MARKET ANALYSIS — GOOGLE SHEETS EDITION  v1.0                ║
║  market_se_gsht.py  — GitHub Actions compatible                  ║
║                                                                            ║
║  Universe  : se_omxlist.csv                                         ║
║  Benchmark : ^OMX                                                ║
║  Timezone  : CET                                                    ║
║  Sheet URL : GOOGLE_SHEET_URL_SE env var                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
SHEET_URL = (
    os.environ.get("GOOGLE_SHEET_URL_SE")
    or "https://docs.google.com/spreadsheets/d/YOUR_SE_SHEET_ID/edit"
)
GSCOPE = [
...
def load_se_universe():
    ...
    df["Sector"] = df["Sector"].map(SE_INDUSTRY_TO_SECTOR).fillna(df.get("Sector","General"))
...
def fetch_se_sector_prices():
    """Fetch SWEDEN sector ETF prices (returns empty dict — no local ETFs)."""
    sector_prices = {}
    for sec, cfg in SE_SECTORS.items():
        tkr = cfg.get("yahoo")
        if not tkr:
            continue
        try:
...
def main():
    ...
    print("\n" + "═"*68)
    print(f"  SWEDEN MARKET — GOOGLE SHEETS EDITION  v1.0")
    ...
    print(f"\n📡 Sweden Sector ETFs …"); sector_prices = fetch_se_sector_prices()
    ...
    print("\n📸 Market Snapshot …");   snap_df    = build_se_snapshot()
    ...
    print("📊 Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, SE_BREADTH_INDICES, INDEX_DATA_DIR, market="SE")
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "SE", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)

    # ── BUILD HTML ──────────────────────────────────────────────────────────
    print("\n🌐 Building Sweden HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "Sweden_Market_Analysis.html")
        build_html_report(
    ...
    write_dashboard_tab(ss, dashboard_df, "SE");             time.sleep(TAB_DELAY)
    ...
    write_sleeve_tab(ss, sleeve_df, "SE");                   time.sleep(TAB_DELAY)
