"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  UAE MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_ae_gsht.py  — NO Google Sheets, generates UAE.html only       ║
║                                                                              ║
║  Universe  : ae_uaelist.csv                                                           ║
║  Benchmark : ^DFMGI                                                         ║
║  Timezone  : GST                                                            ║
║  Output    : UAE.html                                                    ║
║                                                                              ║
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "ae_all_stocks_master.csv")
...
AE_BREADTH_INDICES = {
    "DFM / ADX": {"yahoo": "^DFMGI", "csv": "ae_uaelist.csv"},
}
...
def load_ae_universe():
    csv_path = STOCK_CSV  # full master list
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "ae_uaelist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(AE_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
def build_ae_snapshot():
    ...
    syms = [t["ticker"] for t in AE_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  UAE MARKET — HTML-ONLY EDITION  v1.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M GST')}")
    print(f"  Stocks:{MAX_STOCKS}  Patterns:{PATTERN_MAX}  Signals:{SIGNAL_MAX_STOCKS}  RS:{PRIMARY_RS_PERIOD}d")
    print("="*68+"\n")
    t0 = time.time()

    print(f"\U0001F4C2 Loading UAE universe …"); universe = load_ae_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({AE_INDEX}) …")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(AE_INDEX, start=start_dt.strftime("%Y-%m-%d"),
    ...
    tz_obj   = timezone(timedelta(hours=4))
    run_time = datetime.now(tz_obj).strftime(f"%d %B %Y %H:%M GST")

    print("\n\U0001F4F8 Market Snapshot …");   snap_df    = build_ae_snapshot()
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, AE_BREADTH_INDICES, INDEX_DATA_DIR, market="ae")
    ...
    stock_df = build_stock_strength(
        ...
        market="ae", fetch_financials=FETCH_FINANCIALS,
    ...
    print("\U0001F3C6 Top Picks – Buy …");    top_buy_df  = build_top_picks_buy(
        stock_df, sec_str_df, market="ae", primary_rs=PRIMARY_RS_PERIOD)
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "ae", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="UAE")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("\n\U0001F310 Building UAE HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "UAE.html")
        build_html_report(
            market="ae", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="ae",
    ...
    print(f"\n{'='*68}")
    print(f"  \u2705  COMPLETE!  |  \u23f1 {elapsed:.0f}s  |  \U0001F4C4 UAE.html")
