"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  TURKEY MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_tr_gsht.py  — NO Google Sheets, generates TR.html only       ║
║                                                                              ║
║  Universe  : tr_bist100list.csv                                                           ║
║  Benchmark : XU100.IS                                                         ║
║  Timezone  : TRT                                                            ║
║  Output    : TR.html                                                    ║
║                                                                              ║
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "tr_all_stocks_master.csv")
...
# ─────────────────────────────────────────────────────────────────────────────
#  TURKEY MARKET CONFIG
# ─────────────────────────────────────────────────────────────────────────────

TR_INDEX          = "XU100.IS"
TR_INDEX_FALLBACK = "^XU100"

TR_INDUSTRY_TO_SECTOR = {
...
TR_BREADTH_INDICES = {
    "BIST 100": {"yahoo": "XU100.IS", "csv": "tr_bist100list.csv"},
}

TR_SNAPSHOT_TICKERS = [
    {"name": "BIST 100",                "ticker": "XU100.IS",      "type": "Index"},
    {"name": "Euro Stoxx 50",           "ticker": "^STOXX50E",     "type": "Index"},
    {"name": "DAX 40",                  "ticker": "^GDAXI",        "type": "Index"},
    {"name": "TRY/USD",                 "ticker": "TRY=X",         "type": "Forex"},
    {"name": "USD/TRY",                 "ticker": "USDTRY=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y TurkGB Yield",        "ticker": "^TNX",          "type": "Bond"},
...
def load_tr_universe():
    csv_path = STOCK_CSV  # full master list
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "tr_bist100list.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(TR_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# Turkey sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
TR_SECTORS = {
...
def fetch_tr_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in TR_SECTORS.items():
    ...
    print(f"  ✅ Turkey Sector prices: {len(result)}/{len(TR_SECTORS)}")
    return result
...
def build_tr_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in TR_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  TURKEY MARKET — HTML-ONLY EDITION  v1.0")
    print(f"  {datetime.now().strftime('%d %b %Y  %H:%M TRT')}")
    print(f"  Stocks:{MAX_STOCKS}  Patterns:{PATTERN_MAX}  Signals:{SIGNAL_MAX_STOCKS}  RS:{PRIMARY_RS_PERIOD}d")
    print("="*68+"\n")
    t0 = time.time()

    print(f"\U0001F4C2 Loading Turkey universe …"); universe = load_tr_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({TR_INDEX}) …")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(TR_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  \u26a0 {TR_INDEX} empty, trying {TR_INDEX_FALLBACK} …")
        raw = yf.download(TR_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  \u274c Cannot fetch Turkey index"); return
    cl = raw["Close"]
    ...
    print(f"\n\U0001F4E1 Turkey Sector ETFs …"); sector_prices = fetch_tr_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, TR_SECTORS)
    ...
    run_time = datetime.now(tz_obj).strftime(f"%d %B %Y %H:%M TRT")

    print("\n\U0001F4F8 Market Snapshot …");   snap_df    = build_tr_snapshot()
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, TR_BREADTH_INDICES, INDEX_DATA_DIR, market="tr")
    ...
    stock_df = build_stock_strength(
        ...
        market="tr", fetch_financials=FETCH_FINANCIALS,
    ...
    print("\U0001F3C6 Top Picks – Buy …");    top_buy_df  = build_top_picks_buy(
        stock_df, sec_str_df, market="tr", primary_rs=PRIMARY_RS_PERIOD)
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "tr", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="TR")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("\n\U0001F310 Building Turkey HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "TR.html")
        build_html_report(
            market="tr", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="tr",
    ...
    print(f"\n{'='*68}")
    print(f"  \u2705  COMPLETE!  |  \u23f1 {elapsed:.0f}s  |  \U0001F4C4 TR.html")
