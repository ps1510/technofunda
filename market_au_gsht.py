"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  AUSTRALIA MARKET ANALYSIS — HTML-ONLY EDITION  v1.1 (SIMPLIFIED)      ║
║  market_au_gsht.py  — GitHub Actions compatible                           ║
║                                                                            ║
║  Universe  : au_all_stocks_master_500.csv  (ASX 500 stocks, .AX)         ║
║  Benchmark : iShares Core S&P/ASX 200 ETF (IOZ.AX)                        ║
║  Sectors   : Direct from Industry column in CSV (20 sectors)              ║
║  Timezone  : AEST (ASX market close 16:00 AEST = 06:00 UTC)               ║
║                                                                            ║
║  SIMPLIFIED: Uses Sector column directly from CSV (no complex mappings)   ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "au_all_stocks_master_500.csv")
...
# SECTOR CONFIGURATION — keys MUST match Industry column in au_all_stocks_master_500.csv
# au_all_stocks_master_500.csv uses GICS sector names (11 standard sectors).
# ETF tickers: SPDR ASX sector ETFs (OZF, OZR, etc.) available on Yahoo Finance.
# If an ETF fails to download, market_engine falls back to stock-average RS.
AU_SECTORS = {
    "Financials":             {"yahoo": "OZF.AX",  "csv": "au_sector_financials.csv"},
    "Materials":              {"yahoo": "OZR.AX",  "csv": "au_sector_materials.csv"},
    "Energy":                 {"yahoo": "OZE.AX",  "csv": "au_sector_energy.csv"},
    "Health Care":            {"yahoo": "OZL.AX",  "csv": "au_sector_health_care.csv"},
    "Industrials":            {"yahoo": None,       "csv": "au_sector_industrials.csv"},
    "Technology":             {"yahoo": "OZT.AX",  "csv": "au_sector_technology.csv"},
    "Consumer Discretionary": {"yahoo": "OZD.AX",  "csv": "au_sector_consumer_discretionary.csv"},
    "Consumer Staples":       {"yahoo": "OZS.AX",  "csv": "au_sector_consumer_staples.csv"},
    "Real Estate":            {"yahoo": None,       "csv": "au_sector_real_estate.csv"},
    "Communication Services": {"yahoo": None,       "csv": "au_sector_communication_services.csv"},
    "Utilities":              {"yahoo": "OZU.AX",  "csv": "au_sector_utilities.csv"},
}

AU_BREADTH_INDICES = {
    "S&P/ASX 200":    {"yahoo": "^AXJO",  "csv": "au_asx200list.csv"},
    "S&P/ASX 300":    {"yahoo": "^AXSO",  "csv": "au_asx300list.csv"},
    "S&P/ASX Small":  {"yahoo": "^AXSO",  "csv": "au_asx20list.csv"},
    "S&P/ASX 20":     {"yahoo": "^AXJO",  "csv": "au_asx20list.csv"},
    "S&P/ASX 50":     {"yahoo": "^AXSO",  "csv": "au_asx50list.csv"},
    "S&P/ASX 500":    {"yahoo": "^AXSO",  "csv": "au_all_stocks_master_500.csv"},
}
...
def load_au_universe():
    """
    Load Australia universe from au_all_stocks_master_500.csv.

    CSV structure: Company Name | Industry | Symbol | Series | ISIN Code
    Industry column holds GICS sector names (11 sectors).
    In AU's CSV there is no finer industry sub-classification — Industry IS the sector.
    So: Sector = Industry (direct copy, no mapping needed).

    Verified sector counts (501 stocks total):
      Materials 142 | Financials 122 | Industrials 63 | Consumer Discretionary 42
      Health Care 31 | Technology 31 | Energy 21 | Real Estate 18
      Communication Services 13 | Consumer Staples 8 | Utilities 9
    """
    ...
    # Industry column from CSV = GICS sector name (no mapping required)
    df["Industry"] = df["Industry"].astype(str).str.strip() if "Industry" in df.columns else "Miscellaneous"
    df["Sector"]   = df["Industry"]   # Industry IS the sector in AU's CSV

    df = df[df["Yahoo"].astype(str).str.len() >= 2].copy()
    df = df.reset_index(drop=True)

    sectors = df["Sector"].value_counts()
    print(f"  ✅ AU Universe: {len(df)} stocks | {len(sectors)} sectors")
...
def fetch_au_sector_prices():
    """Fetch sector ETF prices. Returns dict of {sector_name: normalized price Series}."""
    prices = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sector, config in AU_SECTORS.items():
        ticker = config.get("yahoo")
        if not ticker:
            continue
        try:
            ...
            cl = cl.dropna()
            if len(cl) >= 22:
                prices[sector] = _normalize(cl)
        except Exception:
            pass
    print(f"  ✅ Sector ETFs downloaded: {len(prices)}/{len(AU_SECTORS)}")
    return prices
...
def pct_change_n(series, n):
    ...
    return (series.iloc[-1] / series.iloc[-n-1] - 1) * 100 if series.iloc[-n-1]!=0 else np.nan

def build_au_snapshot():
    """Build Australia market snapshot"""
    rows = []
    for t in AU_SNAPSHOT_TICKERS:
        sym = t["ticker"]
        price, chg1, chg5, trend = np.nan, np.nan, np.nan, "N/A"
        try:
            data = yf.download(sym, period="30d", progress=False, auto_adjust=True)
            ...
            if not cdf.empty:
                col = cdf["Close"] if "Close" in cdf.columns else cdf.squeeze()
                if len(col) >= 2:
...
def main():
    ...
    print("\n" + "═"*68)
    print("  AUSTRALIA MARKET — HTML-ONLY EDITION  v1.1 (SIMPLIFIED)")
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, AU_SECTORS)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="AU")
