"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  MEXICO MARKET ANALYSIS — HTML-ONLY EDITION  v1.0                            ║
║  market_mx_gsht.py  — NO Google Sheets, generates MX.html only       ║
║                                                                              ║
║  Universe  : mx_ipclist.csv                                                           ║
║  Benchmark : ^MXX                                                         ║
║  Timezone  : CST                                                            ║
║  Output    : MX.html                                                    ║
║                                                                              ║
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "mx_all_stocks_master.csv")
...
# ─────────────────────────────────────────────────────────────────────────────
#  MEXICO MARKET CONFIG
# ─────────────────────────────────────────────────────────────────────────────

MX_INDEX          = "^MXX"
MX_INDEX_FALLBACK = "MXX"

MX_INDUSTRY_TO_SECTOR = {
...
MX_BREADTH_INDICES = {
    "IPC Mexico": {"yahoo": "^MXX", "csv": "mx_ipclist.csv"},
}

MX_SNAPSHOT_TICKERS = [
    {"name": "IPC Mexico",              "ticker": "^MXX",          "type": "Index"},
    {"name": "Bovespa",                 "ticker": "^BVSP",         "type": "Index"},
    {"name": "S&P 500",                 "ticker": "^GSPC",         "type": "Index"},
    {"name": "MXN/USD",                 "ticker": "MXN=X",         "type": "Forex"},
    {"name": "USD/MXN",                 "ticker": "USDMXN=X",      "type": "Forex"},
    {"name": "DXY (USD Index)",         "ticker": "DX-Y.NYB",      "type": "Forex"},
    {"name": "10Y UST Yield",           "ticker": "^TNX",          "type": "Bond"},
    {"name": "Gold",                    "ticker": "GC=F",          "type": "Commodity"},
    {"name": "Crude WTI",               "ticker": "CL=F",          "type": "Commodity"},
]
...
def load_mx_universe():
    csv_path = STOCK_CSV  # full master list
    if not os.path.exists(csv_path):
        csv_path = os.path.join(INDEX_DATA_DIR, "mx_ipclist.csv")
    if not os.path.exists(csv_path):
    ...
    df["Sector"]   = df["Industry"].map(MX_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# Mexico sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
MX_SECTORS = {
...
def fetch_mx_sector_prices():
    result = {}
    end   = datetime.today() + timedelta(days=1)
    start = end - timedelta(days=PERIOD_DAYS + 5)
    for sec_name, cfg in MX_SECTORS.items():
    ...
    print(f"  ✅ Mexico Sector prices: {len(result)}/{len(MX_SECTORS)}")
    return result
...
def build_mx_snapshot():
    from market_engine import pct_change_n, safe_download
    syms = [t["ticker"] for t in MX_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  MEXICO MARKET — HTML-ONLY EDITION  v1.0")
    ...
    print(f"\U0001F4C2 Loading Mexico universe …"); universe = load_mx_universe()
    if universe.empty: print("\u274c Empty universe — aborting."); return

    print(f"\n\U0001F4E1 Fetching index ({MX_INDEX}) …")
    end_dt  = datetime.today() + timedelta(days=1)
    start_dt= end_dt - timedelta(days=PERIOD_DAYS+5)
    raw = yf.download(MX_INDEX, start=start_dt.strftime("%Y-%m-%d"),
                      end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty:
        print(f"  \u26a0 {MX_INDEX} empty, trying {MX_INDEX_FALLBACK} …")
        raw = yf.download(MX_INDEX_FALLBACK, start=start_dt.strftime("%Y-%m-%d"),
                          end=end_dt.strftime("%Y-%m-%d"), auto_adjust=True, progress=False)
    if raw.empty: print(f"  \u274c Cannot fetch Mexico index"); return
    cl = raw["Close"]
    ...
    print(f"\n\U0001F4E1 Mexico Sector ETFs …"); sector_prices = fetch_mx_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, MX_SECTORS)
    ...
    print("\n\U0001F4F8 Market Snapshot …");   snap_df    = build_mx_snapshot()
    ...
    print("\U0001F4CA Market Breadth …");     breadth_df = build_market_breadth(
        price_data, index_prices, MX_BREADTH_INDICES, INDEX_DATA_DIR, market="mx")
    ...
    stock_df = build_stock_strength(
        ...
        market="mx", fetch_financials=FETCH_FINANCIALS,
    ...
    print("\U0001F3C6 Top Picks – Buy …");    top_buy_df  = build_top_picks_buy(
        stock_df, sec_str_df, market="mx", primary_rs=PRIMARY_RS_PERIOD)
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "mx", run_time,
                                        primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="MX")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("\n\U0001F310 Building Mexico HTML report …")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "MX.html")
        build_html_report(
            market="mx", snapshot_df=snap_df, sector_str_df=sec_str_df,
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="mx",
    ...
    print(f"\n{'='*68}")
    print(f"  \u2705  COMPLETE!  |  \u23f1 {elapsed:.0f}s  |  \U0001F4C4 MX.html")
