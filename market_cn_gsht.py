"""
CHINA MARKET ANALYSIS — HTML-ONLY EDITION  v2.0
market_cn_gsht.py  — NO Google Sheets, generates CN.html only
...
STOCK_CSV = os.path.join(INDEX_DATA_DIR, "cn_all_stocks_master.csv")
...
def load_cn_universe():
    ...
    df["Sector"]   = df["Industry"].map(CN_INDUSTRY_TO_SECTOR).fillna(df["Industry"])
...
# China sector ETFs — no liquid Yahoo-accessible ETFs; synthetic composites via fill_missing_sector_prices
CN_SECTORS = {
...
def fetch_cn_sector_prices():
    ...
    for sec_name, cfg in CN_SECTORS.items():
    ...
    print(f"  ✅ China Sector prices: {len(result)}/{len(CN_SECTORS)}")
    return result
...
def build_cn_snapshot():
    ...
    syms = [t["ticker"] for t in CN_SNAPSHOT_TICKERS]
    try:
...
def main():
    ...
    try:
        ...
        pass
    print("\n" + "="*68)
    print(f"  CHINA MARKET — HTML-ONLY  v2.0")
    ...
    print(f"Fetching index ({CN_INDEX}) ...")
    ...
    sector_prices = fetch_cn_sector_prices()
    ...
    sector_prices = fill_missing_sector_prices(universe, price_data, sector_prices, CN_SECTORS)
    ...
    breadth_df = build_market_breadth(price_data, index_prices, CN_BREADTH_INDICES, INDEX_DATA_DIR, market="cn")
    ...
    top_buy_df  = build_top_picks_buy(stock_df, sec_str_df, market="cn", primary_rs=PRIMARY_RS_PERIOD)
    top_sell_df = build_top_picks_sell(stock_df, sec_str_df, market="cn", primary_rs=PRIMARY_RS_PERIOD)
    chart_df    = build_chart_patterns_df(patterns_list, stock_df, market="cn")
    trade_df    = build_trade_setups(stock_df, sec_str_df, market="cn", primary_rs=PRIMARY_RS_PERIOD)
    sleeve_df   = build_rs_sleeve_list(stock_df, universe, INDEX_DATA_DIR, market="cn", run_time=run_time,
    ...
    dashboard_df   = build_dashboard_df(stock_df, sec_str_df, "cn", run_time, primary_rs=PRIMARY_RS_PERIOD)
    ...
    try:
        ...
        rrg_html = build_rrg_section(_rrg_d, _sl, _rrg_colors(_sl), market_code="CN")
    except Exception as _e:
        print(f"  RRG skipped: {_e}")

    print("Building China HTML report ...")
    try:
        from market_html import build_html_report
        html_path = os.path.join(SCRIPT_DIR, "CN.html")
        build_html_report(
    ...
    try:
        ...
        _db_save(stock_df, sec_str_df, market="cn",
    ...
    print(f"\n{'='*68}")
    print(f"  COMPLETE!  |  {elapsed:.0f}s  |  CN.html")
    print("="*68)
